"""
Dec 5 Election Analysis - DAZZLE EDITION (REVISED V3)
High-impact, high-readability visualizations.
"""
...
VIZ_DIR = DATA_DIR / "gemini_viz"
GEOJSON_PATH = DATA_DIR / "geoBoundaries-HND-ADM1.geojson"
ROOT_DIR = Path(".")

# --- COLORS ---
COLOR_REPORTED = "#2C3E50"    # Midnight Blue (Solid, Trustworthy)
COLOR_ESTIMATED_FILL = "#ffffff" # White fill for hatch
COLOR_ESTIMATED_EDGE = "#F39C12" # Orange edge for hatch
COLOR_ASFURA = "#1F618D"      # Professional Blue
COLOR_NASRALLA = "#C0392B"    # Deep Red
COLOR_MONCADA = "#922B21"     # Darker Red

# --- GLOBAL TEXT ---
TIMESTAMP = "2025-12-05 6:00 pm"
ASSUMPTION_TEXT = "Assumption: Estimated votes follow observed departmental patterns."
...
def add_branding(fig, logo=None, source_text=""):
    """Adds high-quality footer and logo."""
    # Footer Bar
    ...
    # Source Text (Standardized font size)
    fig.text(
        0.02, 0.025,
        source_text,
        ha="left", va="center",
        fontsize=12, color="#555555", fontfamily="Segoe UI" # Standardized fontsize
    )

    # Logo
    if logo is not None:
        logo_ax = fig.add_axes([0.82, 0.005, 0.15, 0.04]) # Adjusted position slightly for better fit
        logo_ax.imshow(logo) 
        logo_ax.axis("off")
    else:
        # Text fallback
...
# --- LOAD DATA ---
from dec5_analysis_and_visuals import load_dec5_results, summarize_departments, load_geojson, add_remaining_votes_to_geojson

def create_dazzle_map(dept_stats_df):
    print("Creating Dazzle Map (V4 Español)...")
    ...
    # Leyenda (Lado derecho de la diapositiva)
    cbar_ax = fig.add_axes([0.92, 0.25, 0.015, 0.5]) # [izquierda, abajo, ancho, alto]
    ...
    out_path = VIZ_DIR / "dazzle_map_remaining.png"
    # Ajustar rect para dar espacio a la nueva posición de la leyenda
...
def create_dazzle_dept_bars(dept_stats_df):
    print("Creando Barras por Depto Dazzle (V4 Español)...")
    logo = load_logo()
    ...
    # Formato de Ejes
    ax.set_yticks(y)
    ax.set_yticklabels(names, fontsize=24, fontweight="700", color="#2C3E50") # Nombres de Depto más grandes
    ...
    for i, (rep, rem) in enumerate(zip(reported, remaining)):
        ...
        if rep > (total * 0.15): 
            ax.text(rep/2, i, rep_text, ha="center", va="center", 
                    color="white", fontsize=20, fontweight="bold") # Texto interior más grande
        
        # Etiqueta Restante
        rem_text = f"Est.\n{rem/1000:.0f}K"
        label_color = COLOR_REPORTED # Texto azul como se solicitó
        
        if rem > (total * 0.15):
            ax.text(rep + rem/2, i, rem_text, ha="center", va="center", 
                    color=label_color, fontsize=20, fontweight="bold", # Texto interior más grande
                    bbox=dict(facecolor="white", alpha=0.85, edgecolor="none", pad=2))
        else:
            ax.text(rep + rem + (total*0.01), i, rem_text.replace("\n", " "), ha="left", va="center",
                    color=label_color, fontsize=22, fontweight="bold") # Texto exterior más grande

    # Título (Centrado, Mejor)
    ...
    out_path = VIZ_DIR / "dazzle_dept_bars.png"
...
def create_dazzle_projection(results_df, dept_stats_df):
    print("Creando Proyección Dazzle (V4 Español)...")
    logo = load_logo()
    ...
    out_path = VIZ_DIR / "dazzle_national_projection.png"
